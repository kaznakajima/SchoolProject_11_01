...
public class Input_Controller_2P : MonoBehaviour
{
    ...
    {
        // 移動量の計算
        float moveX = Input.GetAxisRaw("Horizontal2") * moveSpeed;
        float moveZ = Input.GetAxisRaw("Vertical2") * moveSpeed;

        // 入力判定
        Vector3 inputVec;
        inputVec.x = Input.GetAxisRaw("Horizontal2");
        inputVec.z = Input.GetAxisRaw("Vertical2");
        ...
        {
            if (Input_Controller.myTurn == false)
            {
                MoveSelect();
            }
            else
        ...
        // ターン変更
        if (Input.GetButtonDown("Change2"))
        {
            TurnChange();
        }
    }
